...
import {
  buildPaymentsAppReturnUrl,
  isProbablyMobileUserAgent,
} from '@/lib/payment-return-link'
...
export function ClientPaymentsReturnPage() {
    ...
    return (
      <ClientSurface className="text-center">
        ...
        <p className="mt-3 text-base leading-7 text-muted-foreground">
          We could not find the payment session to resume. Head back to your
          events workspace and try again from there.
        </p>
        <Button
          asChild
    ...
    <div className="space-y-6">
      ...
      <ClientSurface>
        <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
          <div className="space-y-4">
            ...
            {shouldPreferAppHandoff ? (
              <div className="rounded-[1.25rem] border border-border/70 bg-secondary/40 px-4 py-4 text-sm text-muted-foreground">
                We&apos;re trying to reopen Picsa now. If the app stays closed,
                tap{' '}
          ...
          <div className="w-full max-w-md rounded-3xl border border-border/70 bg-background/70 p-5">
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-accent">
              Payment details
            </p>
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          ...
          {session.status === 'SUCCEEDED' && session.eventId ? (
            ...
            >
              <Link to="/app/events/$eventId" params={{ eventId: session.eventId }}>
